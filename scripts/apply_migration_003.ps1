...
if (-not (Test-Path ".\.git\supabase-link")) {
    Write-Warning "⚠️  Projeto não linkado ao Supabase"
...
Write-Host ""
Write-Host "🚀 Aplicando migração 003..." -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0) {
    Write-Error "❌ Falha ao aplicar migração (exit code: $LASTEXITCODE)"
    Write-Host ""
...
Write-Host "✅ MIGRAÇÃO 003 APLICADA COM SUCESSO!" -ForegroundColor Green
Write-Host "======================================" -ForegroundColor Green
Write-Host ""
